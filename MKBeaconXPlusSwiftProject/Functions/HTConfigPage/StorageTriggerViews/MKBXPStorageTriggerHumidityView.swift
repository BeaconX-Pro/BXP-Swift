//
//  MKBXPStorageTriggerHumidityView.swift
...
/// 湿度触发条件配置视图
public final class MKBXPStorageTriggerHumidityView: UIView {
    ...
    public func updateHumidityValue(_ humidity: String) {
        guard !humidity.isEmpty else { return }
        valueButton.setTitle(humidity, for: .normal)
    ...
    private func processSelectMethod(_ index: Int) {
        if index == 0 {
            ...
            noteLabel.text = "*The device stores T&H data when the humodity changed ≥ \(dataList[index])%RH."
        }
        setNeedsLayout()
    }
